...
void CLoop::Book(double lumFactor) {
    ...
    Zpt_reco_topo = new TH1F("Zpt_reco_topo","Zpt reco level for truth-level ZpT<50",400,0,400);
    Zpt_reco_topo_cuts = new TH1F("Zpt_reco_topo_cuts","Zpt reco level for truth-level ZpT<50",400,0,400);
    Zpt_reco_topo_cuts_ptl = new TH1F("Zpt_reco_topo_cuts_ptl","Zpt reco level for truth-level ZpT<50",400,0,400);
}
...
void CLoop::Fill(double weight, int z_sample) {
    ...
    if (n_muons==2){
      ...
      if (q_mu0==q_mu1 && angle<3*pi/4 && trigger_decision && muon_id && trigger_match ) {
        ...
        if(truth_z_pt<50){Zpt_reco_topo->Fill(Z_pt,weight);}
              ...
              if (cuts[3]==1) {
                ...
                if (cuts[4]==1) {
                  ...
                  if (cuts[5]==1) {
                    ...
                    if(truth_z_pt<50){Zpt_reco_topo_cuts->Fill(Z_pt,weight);}
                    ...
                    if(cuts[6]==1){
                      ...
                      if(truth_z_pt<50){Zpt_reco_topo_cuts_ptl->Fill(Z_pt,weight);}
...
void CLoop::Style(double lumFactor) {
    ...
    Zpt_reco_topo->Write();
    Zpt_reco_topo_cuts->Write();
    Zpt_reco_topo_cuts_ptl->Write();
}
