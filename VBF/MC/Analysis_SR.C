...
void CLoop::Book(double lumFactor) {
    ...
    */
    if (lumFactor!=1)
    {
      ...
      h_weight_mc_cuts = new TH1F("weight_mc_cuts","weight total",10000,-500,500);
      h_weight_mc_topo = new TH1F("weight_mc","weight total",10000,-500,500);
    ...
    h_muon_0_iso_TightTrackOnly_FixedRad_topo = new TH1F("muon_0_iso_TightTrackOnly_FixedRad_topo","lepton 1 isolation",2,0,2);
    h_muon_0_iso_TightTrackOnly_FixedRad_topo_dphi_btag_iso2_rnn_ptmu_omega_mreco_tpt = new TH1F("muon_0_iso_TightTrackOnly_FixedRad_topo_dphi_btag_iso2_rnn_ptmu_omega_mreco_tpt","lepton 1 isolation",2,0,2);
    ...
    if (n_muons==1 && n_taus_rnn_loose>=1 && weight > -190){
      ...
      if (ql!=qtau && angle<3*pi/4 && trigger_decision && lepton_id && trigger_match ) {
        ...
        if (signal_events){
          ...
          {
            truth_z_pt=0.0;//truth_Z_p4->Pt()/1000;
          }
          ...
          if (cuts==c_iso||cuts==c_all) {
            h_muon_0_iso_TightTrackOnly_FixedRad_topo_dphi_btag_iso2_rnn_ptmu_omega_mreco_tpt->Fill(muon_0_iso_TightTrackOnly_FixedRad,weight);
          }
          ...
          h_muon_0_iso_TightTrackOnly_FixedRad_topo->Fill(muon_0_iso_TightTrackOnly_FixedRad,weight);
          ...
          if (weight!=1){
            h_weight_mc_topo->Fill(weight,1);
            h_weight_mc_topo->Fill(weight_mc,1);
            if (tau_0_n_charged_tracks==1){
                    ...
                    if (cuts[5]==1) {
                      ...
                      if (cuts[6]==1) {
                        ...
                        if (cuts[7]==1){
                          ...
                          interesting<< to_string(muon_0_p4->Pt())+", "+to_string(muon_0_p4->Eta())+", "+to_string(muon_0_p4->Phi())+", "+to_string(ql)+", "+to_string(muon_0_iso_TightTrackOnly_FixedRad)+", "+to_string(n_muons)+", "+to_string(n_electrons)+", ";
                          ...
                          if (weight!=1){
                            ...
                            h_weight_mc_cuts->Fill(weight_mc,1);
                            h_weight_mc_cuts->Fill(weight,1);
                            h_sf_mu_isolation->Fill(muon_0_NOMINAL_MuEffSF_IsoTightTrackOnly_FixedRad,1);
                            ...
                            h_sf_mu_total->Fill(muon_0_NOMINAL_MuEffSF_HLT_mu26_ivarmedium_OR_HLT_mu50_QualMedium*muon_0_NOMINAL_MuEffSF_IsoTightTrackOnly_FixedRad*muon_0_NOMINAL_MuEffSF_Reco_QualMedium
...
void CLoop::Style(double lumFactor) {
    ...
    */
    ...
    h_muon_0_iso_TightTrackOnly_FixedRad_topo->Write();
    h_muon_0_iso_TightTrackOnly_FixedRad_topo_dphi_btag_iso2_rnn_ptmu_omega_mreco_tpt->Write();
