    ...
    if (n_electrons==1 && n_taus_rnn_loose>=1) {
      ...
      if (ql==qtau && angle<3*pi/4 && trigger_decision && lepton_id && trigger_match){
        ...
        if(signal_events){
          ...
          if ((tau_0_n_isolation_tracks+tau_0_n_core_tracks+tau_0_n_conversion_tracks)>15 || elec_0_iso_FCLoose==0/*elec_0_iso_FCTight==0*/) {
            cuts[3]=1;
          }
            ...
            if (cuts[8]==1) {
              ...
              if (cuts[9]==1) {
                ...
                if (weight!=1){
                  ...
                  h_sf_e_isolation->Fill(elec_0_NOMINAL_EleEffSF_Isolation_TightLLH_d0z0_v13_FCTight/*elec_0_NOMINAL_EleEffSF_Isolation_TightLLH_d0z0_v13_FCLoose*/,1);
